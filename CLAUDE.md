...
- **Tools (`src/tools/es_discovery.py`)**: `ElasticsearchDiscoveryTool` is designed to be registered via `adapter.register_tool(...)`. It implements a **four-stage async hybrid retrieval pipeline**: (1) Elasticsearch BM25 keyword recall, (2) pgvector cosine semantic recall (skipped when `pg_pool=None`), (3) cross-encoder reranking via `sentence-transformers` (soft dep; falls back to score-sort when absent), (4) `max_count_per_domain` diversity cap applied **after** reranking. The `search()` method is `async` and always returns a strict JSON string (`{"results": [...]}` or `{"error": ".."}` — never raises). Constructor: `ElasticsearchDiscoveryTool(es_client, *, pg_pool=None, embedding_func=None, reranker_model="cross-encoder/ms-marco-MiniLM-L-6-v2")`. Existing call sites that pass only `es_client=` remain fully compatible (keyword-only mode).
...
- **Search**: Elasticsearch (`elasticsearch>=9.4.1`); pgvector (via `asyncpg`); `sentence-transformers>=3.0.0` (soft dep, cross-encoder reranking)
